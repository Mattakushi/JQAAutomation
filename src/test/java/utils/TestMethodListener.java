...
public class TestMethodListener implements IInvokedMethodListener {
    ...
    public void beforeInvocation(IInvokedMethod method, ITestResult testResult) {
        ...
        if (method.isTestMethod()) {
            ...
            TestMethodErrorBuffer
                    .set(new ArrayList<Throwable>()); // each test method will have its own error buffer
        }
            ...
            if (lThrowable.size() > 0) {
                ...
                if (size == 1) {
                    ...
                    for (int i = 0; i < size - 1; i++) {
                        ...
                        fullStack.append("Failure ").append(i + 1).append(" of ").append(size)
                                .append("\n");
                        fullStack.append(Utils.stackTrace(lThrowable.get(i), false)[1])
                                .append("\n");
                    }

                    fullStack.append("Failure ").append(size).append(" of ").append(size)
                            .append("\n");
                    Throwable last = lThrowable.get(size - 1);
                    failureMessage.append("(").append(size).append(")")
                            .append(last.getClass().getName()).append(": ").append(last.getMessage())
                            .append("\n\n");

                    fullStack.append(last.toString());

                    testResult.setThrowable(
                            new Throwable(failureMessage.toString() + fullStack.toString()));
